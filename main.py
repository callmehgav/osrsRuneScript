...
# auto rel on
def basicReanimation(self):
    # click each inv slot
    for i in range(4):
        for ii in range(6):
            # Click spell
            moveMouse(returnInt(1212, 1233), returnInt(588, 612))
...
def expertReanimation(self):
    ...
    for i in range(4):
        for ii in range(6):
            # Click spell
            moveMouse(returnInt(1077, 1095), returnInt(648, 672))
            waitClickWait(self, .5, .8)
            # click inv slot
            moveMouse(returnInt(columnNums[i * 2], columnNums[i * 2 + 1]),
                      returnInt(rowNums[ii * 2], rowNums[ii * 2 + 1]))
...
def masterReanimation(self):
    ...
    for i in range(4):
        for ii in range(6):
            # Click spell
            moveMouse(returnInt(1210, 1233), returnInt(652, 679))
            waitClickWait(self, .5, .8)
            # click inv slot
            moveMouse(returnInt(columnNums[i * 2], columnNums[i * 2 + 1]),
                      returnInt(rowNums[ii * 2], rowNums[ii * 2 + 1]))
...
def redwoods(self):
    if (returnInt(0, 10) > 7):
        spec(self)
    moveMouse(returnInt(982, 991), returnInt(284, 500))
    time.sleep(random.uniform(.2, .25))
    for i in range(returnInt(9, 10)):
        mouse.click(self.button)
...
def caveKracken(self):
    ...
    # scepter no praY
    time.sleep(random.uniform(75, 79))
    # click drop
...
def highAlch(self):
    moveMouse(returnInt(1077, 1086), returnInt(637, 648))
    time.sleep(random.uniform(1, 1.2))
    mouse.click(self.button)
    time.sleep(random.uniform(1, 1.2))
    mouse.click(self.button)
